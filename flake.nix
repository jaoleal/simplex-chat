{
  ...
  outputs = { self, haskellNix, nixpkgs, flake-utils, mac2ios, ... }:
    ...
    androidPkgs = {system}: haskellNix.legacyPackages.${system}.appendOverlays [android26];

    pkgs = {system}: haskellNix.legacyPackages.${system};

    buildSimplexLib = { extra-modules, pkgs', ... }: pkgs'.haskell-nix.project {
    ...
    simplexPureBuild = buildSimplexLib { extra-modules = []; inherit androidPkgs; };
    ...
    flake-utils.lib.eachSystem systems (system:
      rec {
        packages = {
            ## androidPkgs is the normal packages but with an override for the android platform
            "lib:simplex-chat" = (buildSimplexLib androidPkgs).simplex-chat.components.library;
        } // ({
            "x86_64-linux" =
              let
                  androidCompiler = androidPkgs.pkgsCross.aarch64-android;
                  android32Compiler = androidPkgs.pkgsCross.armv7a-android-prebuilt;
                  ...
                  # sdk 23.
                  androidFFI = androidCompiler.libffi.overrideAttrs (old: {
                      dontDisableStatic = true;
                      hardeningDisable = [ "fortify" ];
                  });
                  android32FFI = android32Compiler.libffi.overrideAttrs (old: {
                      dontDisableStatic = true;
                      hardeningDisable = [ "fortify" ];
                  });
              ...
              # simplex lib for Windows
              "${androidPkgs.pkgsCross.mingwW64.hostPlatform.system}:lib:simplex-chat" = (buildSimplexLib rec {
                ...
                #Post install of the windows simplex lib
                postInstall = ''
                  set -x
                  function deps() {
              ...
              # STATIC simplex lib for aarch64-linux
              ...
              # lib:support (?) for aarch64-android
              "aarch64-android:lib:support" = (buildSimplexLib androidCompiler).android-support.components.library.override (p: {
                smallAddressSpace = true;
                # no -dynamic
                enableShared = false;
              ...
              # lib for armv7a-android
              "armv7a-android:lib:simplex-chat" = (buildSimplexLib {
                pkgs' = android32Compiler;
                extra-modules = [{
                  packages.text.flags.simdutf = false;
                  packages.direct-sqlcipher.flags.openssl = true;
                  packages.direct-sqlcipher.components.library.libs = pkgs.lib.mkForce [
                    (androidCompiler.openssl.override { static = true; enableKTLS = false; })
                  ];
                  packages.direct-sqlcipher.patches = [
                ...
                postInstall = ''
                  ...
                  # find ${androidCompiler.gmp6.override { withStatic = true; }}/lib -name "*.a" -exec cp {} $out/_pkg \;
                  # find ${androidIconv}/lib -name "*.a" -exec cp {} $out/_pkg \;
                  # find ${androidCompiler.stdenv.cc.libc}/lib -name "*.a" -exec cp {} $out/_pkg \;
                  echo ${androidCompiler.openssl.override { enableKTLS = false; }}
                  find ${(androidCompiler.openssl.override { enableKTLS = false; }).out}/lib -name "*.so" -exec cp {} $out/_pkg \;
              ...
              #lib for android aarch64
              "aarch64-android:lib:simplex-chat" = (simplexPureBuild {
                pkgs' = androidPkgs;
                extra-modules = [{
                  packages.text.flags.simdutf = false;
                  packages.direct-sqlcipher.flags.openssl = true;
